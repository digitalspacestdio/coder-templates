...
mkdir -p /opt/coder
echo '${coder_agent.dev.init_script}' | tee /opt/coder/init
...
docker run -d --rm \
  ...
  -v /home/${lower(data.coder_workspace.me.name)}/envbuilder:/workspaces \
  -e CODER_AGENT_TOKEN="${try(coder_agent.dev.token, "")}" \
  -e CODER_AGENT_URL="${data.coder_workspace.me.access_url}" \
  -e GIT_URL="${data.coder_parameter.repo_url.value}" \
  -e INIT_SCRIPT="echo ${base64encode(try(coder_agent.dev.init_script, ""))} | base64 -d | sh" \
