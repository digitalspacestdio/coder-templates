...
data "coder_parameter" "a260_postgresql_formula" {
  ...
  description  = "Should PostgreSQL be installed?"
  default      = ""
  type         = "string"
...
data "coder_parameter" "a280_mailhog_formula" {
  name         = "a280_mailhog_formula"
  display_name = "Install MailHog"
  description  = "Should MailHog be installed?"
  default      = "mailhog"
  type         = "string"
...
data "coder_parameter" "a560_should_install_kubernetes" {
  ...
  description  = "Should Kubernetes be installed?"
  default      = 0
  type         = "number"
...
resource "coder_app" "mailhog" {
  count        = data.coder_parameter.a280_mailhog_formula.value == "mailhog" ? 1 : 0
  agent_id     = coder_agent.main.id
  slug         = "mailhog"
...
resource "terraform_data" "bootstrap_script_base_system" {
  ...
  if ! which sudo git curl wget jq htop nload vim pv > /dev/null; then
    apt update -qq 
    apt install -yqqq software-properties-common sudo python3 net-tools git-core curl wget jq htop nload vim pv gettext
    add-apt-repository --yes --update ppa:ansible/ansible
    apt install -yqqq ansible
  fi
  ...
  if [[ -d /opt/ansible-playbook ]]; then
    rm -rf /opt/ansible-playbook
  fi

  git clone --depth 1 "https://github.com/cloudspacesh/coder-ansible-php-stack.git" --branch "master" /opt/ansible-playbook
  bash /opt/ansible-playbook/run.sh --tags=base-system,homebrew,zsh,${join(",", local.ansible_playbook_tags)}
  EOT
}
...
locals {
  ...
  ansible_playbook_tags = [
    ...
    data.coder_parameter.a280_mailhog_formula.value
  ]
}
